...
try{	
	...
	con1=DriverManager.getConnection("jdbc:mysql://164.68.105.216:3306/db_GlobalERP","erp","1@erp");

	//conn=DriverManager.getConnection("jdbc:mysql://164.68.105.216/db_leaveapplication","fleetview","1@flv");
	
	conn=DriverManager.getConnection("jdbc:mysql://10.0.10.62/db_leaveapplication","erp","1@erp");
	...
	while(rs.next())
	{
		...
		String check="select * from db_leaveapplication."+session.getAttribute("CompanyMasterID").toString()+"inwarddetails where Subject='"+tokensub[a].trim()+"' and Category='"+tokencat[a]+"' and BillDueDate='"+new SimpleDateFormat("yyyy-MM-dd").format(new SimpleDateFormat("dd-MMM-yyyy").parse(tokendue[a]))+"' ";
		...
		try{
			...
			java.text.DecimalFormat df = new java.text.DecimalFormat("00");
		        ...
		        String sqlIwno="SELECT rid,InwardRefNo  from db_leaveapplication."+session.getAttribute("CompanyMasterID").toString()+"inwarddetails  where InwardRefNo like ('"+refno+""+date+"%') ORDER BY InwardRefNo DESC LIMIT 1";
		...
		try{
			...
			System.out.println(todaysTime);
						 ...
						 String sqlInsert="Insert into db_leaveapplication."+session.getAttribute("CompanyMasterID").toString()+"inwarddetails (InwardNo,InwardRefNo,ManualRef,InwardDate,"+

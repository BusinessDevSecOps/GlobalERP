...
//Image image1 = Image.getInstance("/home/s_kapre/Desktop/UKAS BSI Logo.bmp");
...
preface.setAlignment("center");
preface.add(new Paragraph("sales@transworld-compressor.com    wbsite: www.transworld-compressor.com",NormalFont));
preface.setAlignment("center");
...
preface2.add(new Paragraph("BILLING & DESPATCH OFFICE : Survey No. 921 Kad Vasti,Next to Raisony Collage,Wagholi Domkhel Road,Wagholi,Pune- 412207",forterms));
...
String SqlQuoDet="SELECT * from "+session.getAttribute("CompanyMasterID").toString()+"performainvoicedet WHERE invoiceRefNo='"+invcrefno+"' ";
	...
	firstcell.addElement(new Paragraph("Fax: "+rsInvDet.getString("Fax") +" ",NormalFont));
	firstcell.addElement(new Paragraph("Ph: "+rsInvDet.getString("Phone") +" ",NormalFont));
	tableFirst.addCell(firstcell);

	PdfPCell firstcell1 = new PdfPCell();
	firstcell1.setFixedHeight(80f);

	firstcell1.addElement(new Paragraph("Date : "+new SimpleDateFormat("dd-MMM-yyyy").format(new SimpleDateFormat("yyyy-MM-dd").parse(rsInvDet.getString("InvoiceDate")))+"   ",NormalFont ));
	firstcell1.addElement(new Paragraph("PO No  :  "+rsInvDet.getString("PONO")+"      " ,NormalFont));
	firstcell1.addElement(new Paragraph("PO Date : "+new SimpleDateFormat("dd-MMM-yyyy").format(new SimpleDateFormat("yyyy-MM-dd").parse(rsInvDet.getString("PODate"))) +"    ",NormalFont));
	...
	i=1;
		...
		System.out.println(b);
		   ...
		   h=rsInvItems.getString("SellingPrice") ;
		    ...
		    table.addCell(new Phrase(""+ p +"", FontFactory.getFont(FontFactory.TIMES_ROMAN, 8)));
		    table.addCell(new Phrase(""+ q +"", FontFactory.getFont(FontFactory.TIMES_ROMAN, 8)));
...
paratotal.add(new Paragraph(" Total:(Rs)  "+totalAmount+" ",NormalFont));
paratotal.setAlignment("Right");
document.add(paratotal);


Paragraph parapfi=new Paragraph();
parapfi.add(new Paragraph(" P & F & I (%)  "+rsInvDet.getString("PFI")+" ",NormalFont));
parapfi.setAlignment("Right");
document.add(parapfi);

Paragraph parafright=new Paragraph();
parafright.add(new Paragraph(" FrightAmt "+rsInvDet.getString("FreightAmt")+" ",NormalFont));
parafright.setAlignment("Right");
document.add(parafright);


Paragraph paragtotal=new Paragraph();
paragtotal.add(new Paragraph(" Grand Total "+rsInvDet.getString("GrandTotal")+" ",NormalFont));
...
parabottom5.add(new Paragraph("Subject to Pune Jurisdiction ",NormalFont));
parabottom5.setAlignment("Left");
document.add(parabottom5);


Paragraph parabottom6=new Paragraph();
parabottom6.add(new Paragraph("VAT TIN NO.: 27530091079 V             Pan No.: AABCT4318G               Service Category:Busines Auxiliary Services.  ",NormalFont));
parabottom6.add(new Paragraph("CST TIN NO.: 27530091079 C            Service Tax No.: AABCT4318GST001  " ,NormalFont));
...
parabottom1.add(new Paragraph("For Transworld Compressor Technologies Ltd.",NormalFont));

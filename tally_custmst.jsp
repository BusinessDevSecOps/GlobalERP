...
try 
{
            ...
            //out.println(toDate);
		    String sql="select * from "+session.getAttribute("CompanyMasterID").toString()+"customerdet where tallyname IS NOT NULL";
            String sql1="";
            ...
            <tr >
            <td colspan="7" ><h1>Converted Data to Tally [CUSTOMER MASTER]</h1></td>
            </tr>
            ...
            <tr>
            <%
                String mCompanyname=rs.getString("tallyname").replace("&","&amp;");
                String mBillingadd=rs.getString("BillingAddress");
                String mBillingcity=rs.getString("BillingCity");
                String mBillingstate=rs.getString("Billingstate");
                String mBillingzip=rs.getString("Billingzip");
                String mCstormst=rs.getString("cstormst");
                
                if (mBillingadd!=null)
                {
                  mBillingadd=mBillingadd.replace("&","and");
                }
                String entrysearch = tally.search_in_tables("SELECT * from "+session.getAttribute("CompanyMasterID").toString()+"tallydata_conv where accountname='"+mCompanyname+"' and voucher_type='Customer-Master'");
			...
			xmldoc.append("<STATENAME></STATENAME>\r\n");
			xmldoc.append("<PINCODE>"+mBillingzip+"</PINCODE>\r\n");
			xmldoc.append("<INCOMETAXNUMBER></INCOMETAXNUMBER>\r\n");
			xmldoc.append("<SALESTAXNUMBER>"+mCstormst+"</SALESTAXNUMBER>\r\n");
            ...
            if (senddata.contains("Updated") && (entrysearch=="NOT FOUND") )
            {
            	sql1="insert into "+session.getAttribute("CompanyMasterID").toString()+"tallydata_conv (voucher_type,accountname,username,create_date,tallyvid)"+
                     "VALUES ( 'Customer-Master','"+mCompanyname+"','"+mUsername+"','"+todaysDateTime+"','"+senddata+"' )";

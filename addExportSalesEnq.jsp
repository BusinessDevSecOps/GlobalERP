...
try{
	...
	System.out.println(">>>>> first select sale nu...." +sql1);
	...
	remarks=request.getParameter("remarks")+request.getParameter("specification");
	...
	String sqlInsert=" INSERT INTO "+session.getAttribute("CompanyMasterID").toString()+"salesenq_det ("+
			 ...
			 "	'"+companyName+"','"+rsCustomerdetails.getString("ContactPerson")+"',"+
	...
	System.out.println(">>>>> first insert" +sqlInsert);
	...
	System.out.println(">>>>> first group master insert" +sqlItems);
	...
	while(rsItems.next())
	{
		...
		if(rsItems.getInt("TheGroup")<5)
		  tableToBeAccessed="tbl_"+rsItems.getString("TheGroupName").substring(0,5).toLowerCase()+"im_make"+rsItems.getString("make")+"";
		else 
		...
		System.out.println("ITEM CODE IS Last query>>>  "+item);
		...
		System.out.println(">>>>> last insert  "+SqlInsertDetails);
        ...
        response.sendRedirect("alertGoTo.jsp?msg=Sales Enq added successfully in "+
        		"System with Ref No "+seRefNo+" for "+companyName+".This will redirect you to print this Enquiry &goto=printSalesEnquiry.jsp"
        		+"?senqrefno="+seRefNo);
        con.close();	

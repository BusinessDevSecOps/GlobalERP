...
<body>
 ...
 //conn=DriverManager.getConnection("jdbc:mysql://164.68.105.216/db_GlobalERP","fleetview","1@flv");
 conn=DriverManager.getConnection("jdbc:mysql://164.68.105.216:3306/db_GlobalERP","fleetview","1@flv");
 //conn1=DriverManager.getConnection("jdbc:mysql://164.68.105.216/db_gps","fleetview","1@flv");
...
String sqlnew = "CREATE TABLE IF NOT EXISTS db_GlobalERP."+session.getAttribute("CompanyMasterID").toString()+"billingdetails"+currentyearcount+"_"+finalmonth+" like db_GlobalERP."+session.getAttribute("CompanyMasterID").toString()+"billingdetails"+previousyear+"_"+fpreviousmonth;

System.out.println("sqlnew"+sqlnew);
sterp.executeUpdate(sqlnew);
System.out.println("sqlnew"+sqlnew);
String qr = "select * from db_GlobalERP."+session.getAttribute("CompanyMasterID").toString()+"billingdetails"+currentyearcount+"_"+finalmonth;
System.out.println(qr);
ResultSet rstqr = stqr.executeQuery(qr);
if(!rstqr.next())
{
  System.out.println("in ifff");	
  String sqlinsert="insert into db_GlobalERP."+session.getAttribute("CompanyMasterID").toString()+"billingdetails"+currentyearcount+"_"+finalmonth+"(select * from db_gps.t_billingdetails where month1 = '"+previousyear+"-"+fpreviousmonth+"-01')";
...
if(rschk.next())
{
       ...
       String sqldata="Select ERPCustName as Customer,ERPCode as CustomerCode,SLAPer,Rate,Rate/"+monthdays+" as ChargeperDay,"+
			   ...
			   "where month1='"+spreviousyear+"-"+spreviousmonth+"-01' and  "+session.getAttribute("CompanyMasterID").toString()+"fv_erp_relation"+year+"_"+month+".vehID = "+session.getAttribute("CompanyMasterID").toString()+"billingdetails"+year+"_"+month+".VehID  and "+
...
<td><div align="left"><font size="2" color="black"><a href="javascript:toggleDetails('<%=rsData.getString("Customer") %>','true','<%=i%>')" /><%=rsData.getString("Customer") %></a></font></div>
		<div align="right" class="popup" id="popup<%=i %>">
			    <table width="30%" align="right" border=1 bgcolor="#C6DEFF" cellpadding="0" cellspacing="0" style="position: absolute;">
				    <tr><td><div align="center"><a href="createAnnuxture.jsp?cust=<%=rsData.getString("CustomerCode") %>&name=<%=rsData.getString("Customer") %>&year=<%=year %>&month=<%=month %>&group=<%=rsData.getString("GroupName") %>&vehiclenos=<%=rsData.getString("NoofVehicles") %>&rateperveh=<%=rsData.getString("Rate") %>&jrmcount=<%=rsData.getString("jrmvehcount") %>&billtype=<%=rsData.getString("BillType") %>&slacount=<%=rsData.getString("SLAPer") %>&billdays=<%=rsData.getString("TotalBilledDays") %>"/>Create Annexture</a></div></td></tr>
...
<%
  ...
  else
  {
     ...
     window.location="alertGoTo.jsp?msg=There are no records to display.&goto=vehiclebillreportnew.jsp";
     </script>

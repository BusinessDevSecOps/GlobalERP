	...
	//	java.text.DecimalFormat df = new java.text.DecimalFormat("000");
		...
		System.out.println("++++++++++++++++++++++++++++++++++++++++++======");	
        	...
        	String sqlmain="INSERT INTO "+session.getAttribute("CompanyMasterID").toString()+"invoicedet " +
				 ...
				 stmt.setString(13,rsDet.getString("SalesOrderNo"));
		...
		{ 
			if(null!=request.getParameter("check"+i))
			{
					...
					if(no< 5)
					{
						tableToBeAccessed="tbl_"+groupName.substring(0,5).toLowerCase()+"im_make"+mcode+"";
					...
					String sqlItems=" INSERT INTO  "+session.getAttribute("CompanyMasterID").toString()+"salesinv_items "+
							...
							"VALUES ('"+SINumber+"','"+rsDet.getString("TheGroup")+"',"+
								...
								"'"+sellingprice+"','"+discount+"','"+rsDet.getString("SalesOrderNo")+"'"+
		...
		String sqlUp11="UPDATE  "+session.getAttribute("CompanyMasterID").toString()+"performainvoicedet  set InvoiceGenerated='1'  where TWSalesorderRefNo='"+request.getParameter("SONumber").toString()+"' limit 1  ";
		System.out.println("Proform query is " +sqlUp11);
		st.executeUpdate(sqlUp11);

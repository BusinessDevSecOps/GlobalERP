...
String user="";
	...
	String department1=session.getAttribute("department").toString();
		...
		<%-- <div id="reportData1">		
<iframe id="myIframe" width="100%" height="850px" src="http://164.68.105.216:8080/ERPTestBirt/frameset?__report=Transaction_report.rptdesign&Date=<%=dt %>&Date1=<%=dt1 %>&dt=<%=fromDate1 %>&dt1=<%=dtoday %>&Payment=All&__showtitle=false">
...
System.out.println("----------In Button-------------"+Bt);
        ...
        <thead style="text-align: center;">
            <tr>
            	...
            	<th align="center" style="font-size:12px ; background: #1582AB;  color:white; face:san-serif;"><b>PI</b></th>
            	            	<th align="center" style="font-size:12px ; background: #1582AB;  color:white; face:san-serif;"><b>KRA</b></th>
        ...
        <tbody>
			 ...
			 String sqlTaskAllocation="select Empname as User,dotprojectid as UserID,c.kradotprojectid as KRAid,empcode,HodReport as HOD "
					 +",Dept as Department,IFNULL((select sum(a.task_duration)  from "
							 ...
							 +"task_end_date <='"+dt1+"'  and user_id=c.dotprojectid ),0) "
		      ...
		      while(rs.next()){
		    	  ...
		    	  String sqlTaskAllocation1="select Empname as User,kradotprojectid as UserID,empcode,HodReport as HOD "
							 +",Dept as Department,IFNULL((select sum(a.task_duration)  from "
									 ...
									 +"as Assigned from db_GlobalERP.UserMaster c where c.activestatus='Yes' "
									 +"and c.companymasterid=100000 and c.dept like '%"+DepartmentAll+"%' and c.kradotprojectid='"+KRAid+"' and "
									 +"(c.HODReport like '%"+HODAll+"%' OR c.EmpName like '%"+EmpNameAll+"%' ) order by c.Empname";
			...
			<td align="right" style="font-size: 12px; face:san-serif; color:black"><%=i%></td>
            	...
            	<u><a href="TaskDetRepFrTaskAllo.jsp?userid=<%=UserID %>&fromdate=<%=datenew1 %>&todate=<%=datenew2 %>&HOD=<%=HOD%>&Dept=<%=Dept %>&User=<%=UserName %>&Status=Assigned&role=<%=Sesrole2%>">
            	 <%=AssignedNew%></a></u></td>
            	
            	
            	 <td  align="right" style="font-size: 12px; face:san-serif; color:black">
            	<u><a href="KRADetRepFrTaskAllo.jsp?userid=<%=UserIDKRA %>&fromdate=<%=datenew1 %>&todate=<%=datenew2 %>&HOD=<%=HOD%>&Dept=<%=Dept %>&User=<%=UserName %>&Status=Assigned&role=<%=Sesrole2%>">
            	 <%=AssignedNewKRA%></a></u></td>

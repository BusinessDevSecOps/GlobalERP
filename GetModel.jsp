  ...
  if(action.contains("check"))
  {
	...
	else
	{
		...
		if(flag==true)
		{
		  ...
		  {	  
			  System.out.println("Insert Query"); 
			  String Sinsert="INSERT INTO "+tableName+"(Model,Partno,Description,LandedPrice,EnteredBy,EnteredonDate)values('"+model_name+"','"+partno+"','"+description+"','0','"+session.getAttribute("EmpName").toString()+"','"+today+"')";
			  stmt2.executeUpdate(Sinsert);
			  System.out.println(Sinsert);
	          response.sendRedirect("alertGoTo.jsp?msg=Model,Part,Description Saved Successfully.&goto=MakeMODEL.jsp");
           buffer="#No";
...
else
{
	...
	else
	{
		...
		//tableName="tbl_"+groupName.substring(0,5)+"makemaster";
		...
		String Sinsert="INSERT INTO "+tableName.toLowerCase()+"(ItemCode,Model,Partno,Description,LandedPrice,EnteredBy,EnteredonDate)values('"+itemcode+"','"+model_name+"','"+partno+"','"+description+"','0','"+session.getAttribute("EmpName").toString()+"','"+today+"')";
		stmt2.executeUpdate(Sinsert);
		System.out.println(">>>>>>>>:"+Sinsert);	
		buffer="#Yes";
		out.println(buffer);

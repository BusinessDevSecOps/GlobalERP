...
<body>
 ...
 <%
 	...
 	String selectedName;
 	String FollowUpType = "";
 	int currentmonthcount = 0, currentyearcount = 0, previousmonth = 0, previousyear = 0;
 	String finalmonth = "", fpreviousmonth = "";
%>
<%
	...
	int i=1;
 	...
 	try {
		...
		String sqlNew="CREATE TABLE IF NOT EXISTS db_GlobalERP."+session.getAttribute("CompanyMasterID").toString()+"fv_erp_relation"+currentyearcount+"_"+finalmonth+" like db_GlobalERP."+session.getAttribute("CompanyMasterID").toString()+"fv_erp_relation"+previousyear+"_"+fpreviousmonth;
		...
		{ 
		  ...
		  String sqlinsert="insert into db_GlobalERP."+session.getAttribute("CompanyMasterID").toString()+"fv_erp_relation"+currentyearcount+"_"+finalmonth+"(select * from db_GlobalERP."+session.getAttribute("CompanyMasterID").toString()+"fv_erp_relation"+previousyear+"_"+fpreviousmonth+")";
...
sql="Select distinct(GroupName) from "+session.getAttribute("CompanyMasterID").toString()+"fv_erp_relation"+currentyearcount+"_"+finalmonth;
...
<% 	
	...
	sql2="select * from "+session.getAttribute("CompanyMasterID").toString()+"fv_erp_relation"+currentyearcount+"_"+finalmonth+" where GroupName='"+request.getParameter("transporter")+"'";
...
<%
sql3="select * from "+session.getAttribute("CompanyMasterID").toString()+"fv_erp_relation"+currentyearcount+"_"+finalmonth+" where GroupName='"+request.getParameter("transporter")+"'";
...
<td><div align="left"><input type=button name="Yes" value="Yes" id="Yes" onclick="getdata('<%=rs3.getString("VehRegno")%>','<%=rs3.getString("Transporter") %>','<%=rs3.getString("GroupName") %>','Yes');"> / <input type=button name="No" value="No" id="No" onclick="getdata('<%=rs3.getString("VehRegno")%>','<%=rs3.getString("Transporter") %>','<%=rs3.getString("GroupName") %>','No');"> </div></td>
</tr>
<%
}
%>

...
int counter=0;
  ...
  out.print("<input type='hidden' name='Code"+i+"' id='Code"+i+"' value='"+code+"'>");
  
    
  if(newstring.toLowerCase().contains(company.toLowerCase()))
  {
  buffer+="<tr><td align='left'><input  name='chkbx' id='ab"+i+"' type='checkbox' value='"+name+"*"+code+"' /> "+rs1.getString("a").replaceAll("(?i)"+company,"<font color=blue><u>"+company.toUpperCase()+"</u></font>")+".</td></tr>";
  }
  ...
  buffer+="<input type='hidden' name='element' id='element' value="+limitcnt+">";
   ...
   if(limitcnt!=0)
  { 
   	   buffer+="<tr><td align='left'><h3><a href='#' onclick='getCompanies();' >more..</a></h3></td></tr>";
  }
   else
  {  

...
try
{
	...
	//Connection connForMail=DriverManager.getConnection("jdbc:mysql://164.68.105.216/db_leaveapplication","fleetview","1@flv");
	///Connection connForMail=DriverManager.getConnection("jdbc:mysql://10.0.10.62/db_leaveapplication","fleetview","1@flv");
	...
	String SqlEmpDetails="select Dept,TALevel,HodReport from "+session.getAttribute("CompanyMasterID").toString()+"security where EmpName='"+EmpName+"'";
	...
	if(rsEmpDetails.next())
	{
	   ...
	   String SqlData="select email,HodReport,Dept from "+session.getAttribute("CompanyMasterID").toString()+"security where EmpName='"+EmpName+"'";
	  ...
	  //  String SqlHod="select email from "+session.getAttribute("CompanyMasterID").toString()+"security where Dept='"+Dept+"' and Level='Level5'";
	   ...
	   ResultSet rsHod=st6.executeQuery(SqlHod);
	...
	if(id.contains("-"))
	{
	 ...
	 System.out.println("TO  DATE IS:    "+todate);
		...
		else
		{
			...
			ResultSet rsClaim=st1.executeQuery(SqlClaim);
			while(rsClaim.next())
			{
				   /*if(Client=="")
				   {
...
//	System.out.println(">>>>advanceTaken:"+advanceTaken);  
	...
	if(button.equals("0"))
	{
		...
		if(id.contains("-"))
		{
			...
			System.out.println(">>>>>>>>sqlClaim:"+sqlClaim);
			ResultSet rssqlClaim=st1.executeQuery(sqlClaim);
			if(rssqlClaim.next())
			{
			...
			{*/
				...
				System.out.println("IN SAVE   IN SAVE   IN SAVE   ");
				  ...
				  "CostCenter,Status ,CommentsByEmp,CommentsByHOD,CommentsApprovedBy,Origin,Destination,CommentsByACDepa,Cancel,HodReport,UploadBill"+
				  ") VALUES ("+
				 ...
				 // response.sendRedirect("NewClaim.jsp?id="+clmrefid+"&status="+status+"&rs="+msg+"&action="+action+"");
				  response.sendRedirect("alertGoTo.jsp?msg=Claim Save with ID "+clmrefid+" &goto=claimReport.jsp?action=view ");
		...
		else
		{
			if(action.contains("update"))
			{
            	...
            	*/
            	...
            	if(itemNames!=null)
            	{	
            	SqlUpdateClaim=" Update "+session.getAttribute("CompanyMasterID").toString()+"claim_master Set SumOfClaimAmount='"+ SumOfAmt+"',ClaimType='"+claimtype1+"',DepartureDateTime='"+fromdate2+" 00:00:00',ArrivalDateTime='"+todate2+" 00:00:00',Purpose='"+purpose1+"',AdvanceTaken='"+advanceTaken1+"',CostCenter='"+ourcompany1+"',Origin='"+origin1+"',Destination='"+destination1+"',UploadBill='"+itemNames+"' where  CLMRefNumber='"+id+"' and Cancel='1' ";
            	}
            	else
            	{
            		SqlUpdateClaim=" Update "+session.getAttribute("CompanyMasterID").toString()+"claim_master Set SumOfClaimAmount='"+ SumOfAmt+"',ClaimType='"+claimtype1+"',DepartureDateTime='"+fromdate2+" 00:00:00',ArrivalDateTime='"+todate2+" 00:00:00',Purpose='"+purpose1+"',AdvanceTaken='"+advanceTaken1+"',CostCenter='"+ourcompany1+"',Origin='"+origin1+"',Destination='"+destination1+"' where  CLMRefNumber='"+id+"' and Cancel='1' ";
	...
	{    //coding for submit button
	   ...
	   //******************** End  Total claim Amount & paybale Amt ***************
		...
		{ 
			...
			else
			{
				...
				System.out.println(">>>>>>>>sqlClaim:"+sqlClaim);
				ResultSet rssqlClaimM=st1.executeQuery(sqlClaim);
				if(rssqlClaimM.next())
				{
				...
				if(count1==0)
				{
					 sqlInsert="INSERT INTO "+session.getAttribute("CompanyMasterID").toString()+"claim_master ("+
					  ...
					  "'"+todate+" 00:00:00','"+purpose+"','"+todaysDateTime+"','Web Site','"+todaysDateTime+"', "+
					  ""+ SumOfAmt+" ,"+
					  "'"+advanceDisp+"','"+ourcompany+"','Pending','-','-','-','"+origin+"','"+destination+"','-','1','"+HodReport+"','"+itemNames+"')";
			...
			else
			{
				...
				else
				{
				 ...
				 System.out.println(">>>>>>>>>>>EmpClaim:"+EmpClaim);
				  ...
				  ResultSet rsEmpClaim=st1.executeQuery(EmpClaim);
				  if(rsEmpClaim.next())
				  {
	   ...
	   //    SqlClaimAmount="Select SUM(Amount) from "+session.getAttribute("CompanyMasterID").toString()+"claim_det where CLMRefNumber='"+id+"' and Cancel=1";
		   ...
		   ResultSet rsClaimDet=st1.executeQuery(SqlClaimDet);
		   ResultSet rsClaimDet1=st5.executeQuery(SqlClaimDet);

...
<title>Insert title here</title>
    ...
    <body>
    <form name="xys" method="get" action="">
                  File Name   <input type="text" id="filename" name="filename" value="c:\\tally.erp9\\creditmemo.xml" size = "50"/>
        ...
        Writer output = null;
        String text = "";
        File file = new File("C:\\Documents and Settings\\Shekhar\\Desktop\\CreditNote.xml");
        output = new BufferedWriter(new FileWriter(file));
        try {
            String fromDate="2011-09-10";
            String toDate="2011-09-15";
            Class.forName("org.gjt.mm.mysql.Driver");
            Connection conn1= DriverManager.getConnection("jdbc:mysql://192.168.1.55/erp_vpedutech","fleetview","1@flv");
            Statement st1=conn1.createStatement();
            String sql="select * from "+session.getAttribute("CompanyMasterID").toString()+"allpaymentdetails as a INNER JOIN "+session.getAttribute("CompanyMasterID").toString()+"customerdet as b on a.CustomerCode = b.customerCode where a.paymentdate BETWEEN '"+fromDate+"' and '"+toDate+"'  and paymentmode = 'Credit Note' order by paymentdate";
        ...
        <table border="2">
            ...
            while(rs.next()) {
                ...
                String mSecondname=""; // ACCOUNT NAME OF DEBIT ACCOUNTS....
                String mAmount=rs.getString("amountpaid");
                String mNarration=rs.getString("remarks");
                String mReference = " ";
                //Being Crs, note pass against for 10% TDS deduction against invoice no. SI020411095 as per SCN200411001
            ...
            output.write(text);
            text="     <VOUCHER  VCHTYPE='Credit Note' ACTION='Create'>\r\n";
            output.write(text);
            text = "<DATE>"+mVoudate+"</DATE>\r\n";
            output.write(text);
            //text="      <GUID>534280e0-8c06-11d9-a6f7-525405f8aba3-000142da</GUID>\r\n";
            //output.write(text);
            text="      <NARRATION>"+mNarration+"</NARRATION>\r\n";
            output.write(text);
            text="      <VOUCHERTYPENAME>Credit Note</VOUCHERTYPENAME>\r\n";
            output.write(text);
            ...
            output.write(text);
            text="        <BILLCREDITPERIOD>10 Days</BILLCREDITPERIOD>\r\n";
            output.write(text);
            text="        <SERVICETAXLEDGER>Service Tax (Output)</SERVICETAXLEDGER>\r\n";
            output.write(text);
            ...
            text="      <ALLLEDGERENTRIES.LIST>\r\n";
            output.write(text);
            text="       <LEDGERNAME>TDS 2011-12 (Assets )</LEDGERNAME>\r\n";
            output.write(text);
            text="       <GSTCLASS/>\r\n";
            output.write(text);
